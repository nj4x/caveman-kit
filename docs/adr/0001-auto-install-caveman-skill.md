...
**Status:** Accepted (amended 2026-08-30: auto-install is now the default; opt out with `--no-install-skill`)
...
- **Trust boundary (superseded 2026-08-30):** No prompt is shown before the third-party skill (JuliusBrussee/caveman) is fetched and injected — `install.sh` runs `npx skills add` against it by default. Users who want the old consent gate must remember `--no-install-skill` themselves; the kit no longer surfaces the choice.
